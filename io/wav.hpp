...
struct WAVHEADER
{
    ...
    std::ostream& write(std::ostream& str)
    {
        auto& wh = *this;
...
auto generate_wav(InputIt begin, InputIt end, T max_value, double sample_rate = 44100)
{
    ...
    while (begin != end)
    {
        short v = *(begin++) * std::numeric_limits<short>::max() / max_value;
        ss.write((char*)&v,2);
    }
...
auto generate_wav(Container c, T max_value,double sample_rate)
{
   return generate_wav(std::begin(c),std::end(c),max_value,sample_rate);
}
