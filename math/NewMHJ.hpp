...
template <typename Function, typename T = float>
float NewMHJ(std::vector<T>& x,Function f)
{
	int N = x.size();
    int n_count=1000; // максимальное число итераций
	int i,count,k;
    double eps_proc=1.e-11,eps_opt=1.e-9; // точность вычислений
